...
// struct that represents a message in the Vector clock system.
type Message struct {
	from      int
...
// struct that represents an event in the Vector clock system, logged for the final ordering of events.
type Event struct {
	processId int
	clock     VectorClock
	kind      EventType
...
func (l *Logger) find(label string) VectorClock {
...
func (p *Process) receive() {
	...
	p.mu.Lock()
	for i := 0; i < N; i++ {
		if msg.timestamp[i] > p.clock[i] {
			p.clock[i] = msg.timestamp[i]
		}
	}
	p.clock[p.id-1]++
	p.mu.Unlock()
...
func relationship(a, b VectorClock) string {
	if a == b {
		return "same event"
	}
	switch {
...
func main() {
	...
	for _, event := range logger.events {
		fmt.Printf("P%d  clock=%v  %s 	 %s\n", event.processId, event.clock, eventTypeNames[event.kind], event.detail)
	}

	fmt.Println("\nConcurrent relationship:")
	bootClock := logger.find("boot")
	processingClock := logger.find("processing")
	fmt.Printf("boot vs processing: %s\n", relationship(bootClock, processingClock))

	fmt.Println("\nCausal relationship:")
	sendClock := logger.find("\"hello P2, I am P1\" -> P2")
	receiveClock := logger.find("\"hello P2, I am P1\" from P1")
	fmt.Printf("send vs receive: %s\n", relationship(sendClock, receiveClock))
}
